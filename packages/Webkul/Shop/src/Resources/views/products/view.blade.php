...
@php
    ...
    $waNumber = preg_replace('/\D+/', '', $contactNumber);
    if (preg_match('/^(?:0|254)?(7\d{8}|1\d{8})$/', $waNumber, $m)) {
        $waNumber = '254' . $m[1];
    }
    $waProductUrl = 'https://wa.me/' . $waNumber . '?text=' . rawurlencode("Hi! I'd like to order: " . $product->name . " — " . url()->current());
@endphp
                            ...
                            <div class="relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                ...
                                @if ($product->isSaleable(1))
                                    <a
                                        ...
                                        class="mt-3 flex w-full max-w-[470px] items-center justify-center gap-2.5 rounded-2xl bg-[#25d366] px-6 py-4 text-base font-semibold text-white shadow-sm transition-colors hover:bg-[#1ebe5d] max-md:py-3 max-sm:mt-2 max-sm:rounded-lg max-sm:py-1.5 max-sm:text-sm"
