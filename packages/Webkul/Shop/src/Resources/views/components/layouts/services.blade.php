...
@php
    ...
    $serviceCopyOverrides = [
        ...
        'Product Replace' => [
            ...
            'description' => 'Straightforward replacement support if your wellness device arrives damaged or faulty.',
        ],
        'Emi Available' => [
            'title'       => 'Flexible Payment Options',
            'description' => 'Pay conveniently with M-Pesa, cards, and other supported checkout options.',
        ],
        '24/7 Support' => [
            'title'       => 'Expert Customer Support',
            'description' => 'Get help choosing the right wellness products before and after your purchase.',
        ],
    ];
@endphp
    ...
    >
        <div class="max-md:max-y-6 flex justify-center gap-6 max-lg:flex-wrap max-md:grid max-md:grid-cols-2 max-md:gap-x-2.5 max-md:text-center">
            @foreach ($customization->options['services'] as $service)
                @php
                    $serviceContent = $serviceCopyOverrides[$service['title']] ?? [
                        'title'       => $service['title'],
